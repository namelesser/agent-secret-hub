...
# 读取项目版本
PROJECT_VER=$(grep '^version' "$PROJECT_DIR/pyproject.toml" | head -1 | grep -oP '"\K[^"]+')

# 读取 skill 版本
SKILL_VER=$(grep -oP 'v[0-9]+\.[0-9]+\.[0-9]+' "$SKILL_DIR/agent-secret-hub/SKILL.md" | head -1)

if [ "$PROJECT_VER" = "${SKILL_VER#v}" ]; then
    echo "✓ 版本一致: v$PROJECT_VER"
    exit 0
fi

echo "⚠ 版本不一致: 项目=v$PROJECT_VER, skill=$SKILL_VER"
echo "==> 更新 skill..."
...
NEW_VER=$(grep -oP 'v[0-9]+\.[0-9]+\.[0-9]+' "$SKILL_DIR/agent-secret-hub/SKILL.md" | head -1)
